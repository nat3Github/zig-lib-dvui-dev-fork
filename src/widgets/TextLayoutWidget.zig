...
const bidi = @import("TextLayoutWidget/bidi.zig");
const caret = @import("TextLayoutWidget/caret.zig");
...
fn selMovePre(self: *TextLayoutWidget, f: Fragment, index: usize) void {
    ...
    switch (self.sel_move) {
        ...
        .mouse => |*m| {
            if (m.down_pt) |p| {
                if (caret.hitHere(self, p, index)) |ba| {
                    m.byte = ba.byte;
...
fn reorderLineVisual(self: *TextLayoutWidget) void {
    ...
    const pieces = bidi.bidiPieces(arena, frags, &base_direction) orelse return;
    ...
    bidi.assignVisualX(arena, pieces, out.items, frags[0].x);
    self.line_frags = out;
}
...
fn flushLine(self: *TextLayoutWidget) void {
    ...
    bidi.reshapeWithNeighbourContext(self.line_frags.items, self.baseDir());
    ...
    self.line_end_byte = caret.lineEndByte(self.line_frags.items);
...
fn emitFragment(self: *TextLayoutWidget, f: Fragment, index: usize) void {
    ...
    if (self.selection.start >= f.bytes_seen and self.selection.start <= f.bytes_seen + f.text.len) {
        ...
        self.sel_start_r_new = .{ .x = caret.fragCaretX(f, off), .y = f.y, .w = 1, .h = f.size.h };
    ...
    if (self.selection.end >= f.bytes_seen and self.selection.end <= f.bytes_seen + f.text.len) {
        ...
        self.sel_end_r_new = .{ .x = caret.fragCaretX(f, off), .y = f.y, .w = 1, .h = f.size.h };
    ...
    if (!self.cursor_seen and (self.selection.cursor < f.bytes_seen + f.text.len or (self.selection.cursor == f.bytes_seen + f.text.len and self.selection.affinity == .before))) {
        ...
        self.cursor_rect = Rect{ .x = caret.fragCaretX(f, cursor_offset), .y = f.y, .w = 1, .h = f.size.h };
    ...
    if (!self.cursor_seen) {
        ...
        self.cursor_rect = Rect{ .x = caret.fragCaretX(f, f.text.len), .y = f.y, .w = 1, .h = f.size.h };
    }
