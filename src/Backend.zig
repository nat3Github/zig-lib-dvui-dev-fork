...
/// within the given rect. Only exists when `has_texture_update_sub_rect`.
pub const textureUpdateSubRect = if (has_texture_update_sub_rect) textureUpdateSubRectImpl else {};

pub const has_texture_update_sub_rect = @hasDecl(Implementation, "textureUpdateSubRect");

fn textureUpdateSubRectImpl(self: Backend, texture: dvui.Texture, pixels: [*]const u8, x: u32, y: u32, w: u32, h: u32) TextureError!void {
